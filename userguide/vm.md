...
6. 选择并配置虚拟机基础管理配置，包括虚拟机名称、登录方式、登录密码（可选择随机生成）和项目组信息等。
...
7. 配置高级设置，包括主机名、USB、DNS、CPU启动模式，高可用模式，隔离组。
...
8. 选择购买数量和付费方式，如下图所示确认订单并点击“立即购买” 进行虚拟机创建操作。
...
虚拟机列表页可查看当前账户下已有的虚拟机资源列表，包括名称、资源ID、状态、VPC、子网、集群与特性、配置、CPU使用率、内存使用率、磁盘使用率、IP地址、项目组、高可用级别、计费方式、创建时间、过期时间及操作等，同时也可通过“自定义列表”按钮，自定义列表所需信息。
...
* 高级设置包括主机名，cpu模式，高可用级别，隔离组，点击高可用级别修改按钮可变更高可用级别；
