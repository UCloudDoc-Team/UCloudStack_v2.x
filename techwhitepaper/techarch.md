...
### 3.1.4 vGPU 透传
UCloudStack 平台支持 vGPU 设备透传能力，为平台用户提供 vGPU 虚拟机服务，让用户共享资源，让虚拟机拥有高性能计算和图形处理能力。
普通GPU设备使用是采用(透传)的方式给虚拟机使用的，而vgpu是将GPU划分成更小的 逻辑单元给虚拟机使用。

vGPU 虚拟机与标准虚拟机采用一致管理方式，包括内外网 IP 分配、弹性网卡、子网及安全组管理，并可对 vGPU 虚拟机进行全生命周期管理，包括重置密码，变更配置及监控等，使用方式与普通的虚拟机一致，支持多种操作系统，如 CentOS、Ubuntu、Windows 等，在不增加额外管理的基础上，为租户提供快捷的 vGPU 计算服务。

vGPU操作系统支持情况：
![](../images/techwhitepaper/vgpu1.png)

!> 平台默认不支持 GPU 虚拟化，如需 GPU 虚拟化能力，需要对GPU所在物理机安装相应驱动，并购买 GPU 虚拟化授权，才能创建vGPU虚拟机。

vGPU驱动选择（由于新版本支持的gpu型号更多，因此我们选择安装了最新版本）：
![](../images/techwhitepaper/vgpu2.png)
...
### 3.2.2 离线迁移
离线迁移，是虚拟机在关机状态下，在不同的集群之间进行的离线跨集群迁移。离线迁移不涉及存储及数据迁移，更改虚拟机的集群配置，在虚拟机下次启动的时候，重新进入启动流程，重新分配物理机，在目标物理机注册一个相同配置的虚拟机进程，然后进行虚拟机内存数据同步，最终重新启动虚拟机到目标物理机。适用于云平台资源动态调整、物理机停机维护、优化服务器能源消耗等场景，进一步增强云平台可靠性。
