...
- 订单管理000：查看平台发生的所有订单记录，或者或者自定义查询时间段对订单记录进行查询。
...
平台管理员可在订单管理页面，可通过自定义所属地域和查询时间查看云平台在一定时间周期内产生的订单，如下图所示：
...
平台管理员可在交易管理页面，可通过自定义所属地域和查询时间查看云平台在一定时间周期内产生的交易信息，如下图所示：
...
价格配置即平台全局的产品定价，平台支持对 6 个维度的资源项进行定价，包括 CPU、内存、磁盘、外网IP、GPU。单个云服务的实际出售价格，根据云服务所涉及到的计费资源项进行累加。租户的价格默认继承价格配置中的价格，可由管理员自定义租户价格的折扣，以适应平台运营的需求。
