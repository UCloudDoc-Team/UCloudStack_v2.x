...
1. 登录控制台并进入账号控制台，可通过 “账号和组织管理” - “我的账号” 中的 “账号安全” - “登录保护” 入口进行操作，如下图所示：
   ...
   ![](../images/adminguide/login.png)

   点击下一步，国密硬件版操作如下图，输入硬件SN，不可为空：

   ![](../images/adminguide/login01.png)

   普通软件版操作如下：

   ![](../images/adminguide/loginprotect.png)
...
![](../images/adminguide/fortitokenlogin.png)
...
![](../images/adminguide/LoginWhitelist1.png)
...
![](../images/adminguide/updateadminemail.png)
...
![](../images/adminguide/adminapisec.png)
...
租户可由平台系统管理员创建，创建租户时默认会生成一个用户账号，此账号即为租户的主账号，也是租户下的管理人员。注册成功后需要管理员进行充值才可进行使用。平台系统管理员可对平台的所有租户进行生命周期管理，包括创建租户、激活账号、修改名称、冻结、解冻、登录访问限制、开关审批流程、地域授权、修改邮箱、删除租户。并可对租户进行主/子账号管理、配额管理 、订单管理、交易管理、充值管理、价格管理及资源概览查看，详见 [租户管理](/UCloudStack_v2.x/adminguide/tenant.md)。
