...
| **CompanyID** | int | 租户ID，仅admin操作时生效 | No |
| **Keyword** | string | 关键字 | No |
| **RuleIDs.N** | array<string> | 【数组】NAT网关白名单ID。调用方式举例：RuleIDs.0=“one-id”、RuleIDs.1=“two-id”。 | No |
...
| **Message** | string | 返回错误消息，当 `RetCode` 非 0 时提供详细的描述信息 |
| **TotalCount** | int | 总条目数 |
| **Infos** | array<[*NATGWRuleInfo*](#NATGWRuleInfo)> | NATGWRule信息列表 |
...
| **RuleID** | string | 白名单ID |
| **NATGWType** | string | nat网关类型 |
| **BindResourceID** | string | 绑定的资源ID |
| **BindResourceType** | string | 绑定资源的类型 |
| **Address** | string | 绑定资源的IP信息，对应于资源类型，分别为虚拟机内网ip、subnet网段、vpc网段 |
| **EIP** | string | 出口IP的地址 |
| **Name** | string | 添加的白名单资源名称 |
| **IP** | string | 同Address(拟弃用) | 
| **RuleStatus** | string | 状态，Bounding:绑定中,Bound:已绑定,Unbounding:解绑中,Unbound：已解绑 |
