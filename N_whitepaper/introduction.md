...
- 镜像：虚拟机运行时所需的操作系统，提供CentOS、Windows、Ubuntu、Redhat、SUSE、BC-linux、OpenEuler等常用基础操作系统镜像；支持将虚拟机导出为镜像，通过自制镜像重建虚拟机；同时支持镜像的导入导出，便于用户自定义镜像；同时兼容ARM生态的UOS、银河麒麟等服务器操作系统;
